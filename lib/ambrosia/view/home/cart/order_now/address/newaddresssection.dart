...
class AddressSection extends StatefulWidget {
  final bool isFromManageAddress;
  final Address? address; // Add this for edit mode
  final VoidCallback? onAddressUpdated; // Add this for edit mode

  const AddressSection({
    Key? key,
    this.isFromManageAddress = false,
    this.address, // Make optional for add mode
    this.onAddressUpdated, // Make optional for add mode
  }) : super(key: key);
...
class _AddressSectionState extends State<AddressSection> {
  ...
  void _initializeFormData() {
    if (_isEditMode) {
      // Pre-fill form with existing address data for edit mode
      ...
      // For add mode, you can pre-fill with user data if needed
      final userProvider = Provider.of<UserProvider>(context, listen: false);
      _fnameController.text = userProvider.fname;
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 200 && responseData['status'] == true) {
          ...
          widget.onAddressUpdated?.call(); // Refresh the address list
  ...
  Future<void> _saveAddress() async {
    if (_formKey.currentState!.validate()) {
      ...
      bool success = await addressProvider.saveCheckoutInformation(
        address_type: _addressType.toString(),
        district: _districtController.text,
        userid: userId,
        fname: _fnameController.text,
