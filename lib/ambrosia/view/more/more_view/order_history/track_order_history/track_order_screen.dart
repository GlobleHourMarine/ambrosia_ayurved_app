...
class TrackingScreen1 extends StatefulWidget {
  final String orderId; // Changed from awbCode to orderId

  const TrackingScreen1({Key? key, required this.orderId}) : super(key: key);
...
class _TrackingScreen1State extends State<TrackingScreen1>
  ...
  Future<void> _trackShipment() async {
    try {
      ...
      var request = http.Request(
        'GET',
        Uri.parse(
            'https://ambrosiaayurved.in/api/track_order_status?order_id=${widget.orderId}'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true &&
          ...
          setState(() {
            _error = 'No tracking information found for this order.';
            _isLoading = false;
          });
        }
      } else {
  ...
  // Updated to handle your backend status terms
  int _getCurrentStepIndex(String currentStatus) {
    ...
    // Step 2: Shipped/Dispatched/In Transit
    ...
    if (status.contains("cancellation requested") ||
      ...
      return 0; // Keep at order confirmed for cancelled orders
    }
  ...
  String _getStatusMessage(String currentStatus) {
    ...
    // Handle special statuses
    if (status.contains("cancellation requested")) {
      return "Your cancellation request is being processed";
    }
  ...
  Widget _buildOrderHeader(Map<String, dynamic> orderData) {
    final currentStatus = orderData['current_status'] ?? 'Processing';
      ...
      child: Padding(
        ...
        child: Column(
          children: [
                  ...
                  children: [
                    Icon(
                      _getCurrentStepIndex(currentStatus) == 4
                          ? Icons.check_circle
                          : currentStatus.toLowerCase().contains('cancel')
                              ? Icons.cancel
            ...
            if (orderData['expected_delivery_date'] != null)
              Padding(
                padding: EdgeInsets.only(top: 10),
                child: Text(
                  'Expected delivery: ${_formatDate(orderData['expected_delivery_date'])}',
                  style: TextStyle(
                    color: statusColor,
                    fontSize: 14,
                  ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 15),
          _buildDetailItem(Icons.confirmation_number, 'Order ID',
              orderData['order_id'] ?? 'N/A'),
          _buildDetailItem(Icons.confirmation_number, 'AWB Code',
              orderData['awb_code'] ?? 'N/A'),
          _buildDetailItem(Icons.local_shipping, 'Courier',
              orderData['courier_company'] ?? 'N/A'),
          _buildDetailItem(Icons.person, 'Customer Name',
              '${orderData['fname'] ?? ''} ${orderData['lname'] ?? ''}'),
          _buildDetailItem(Icons.phone, 'Mobile', orderData['mobile'] ?? 'N/A'),
          _buildDetailItem(Icons.location_on, 'Delivery Address',
